...
class MasterServices {
    async getCommunityStatsAsync() {
        try {
            const [{ count: heroesCount }] = await knex("nangal_heroes").count('hero_id as count').where('is_deleted', false);
            const [{ count: donorsCount }] = await knex("donors").count('donor_id as count').where('is_deleted', false);
            const [{ count: eventsCount }] = await knex("events").count('eventId as count').where('isdeleted', false).andWhere('isactivated', true);
